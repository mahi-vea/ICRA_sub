...
void publish_tracked_objects() {
  ...
  for (int i = 0; i < NUM_FILTERS; i++) {
    float x = KF[i].statePre.at<float>(0);
    float y = KF[i].statePre.at<float>(1);
    ...
    if (vx == 0.0f && vy == 0.0f) {
      ...
      del_text.header.frame_id = "front_laser";
    ...
    text_marker.header.frame_id = "front_laser";
...

{
  ...
  if (firstFrame) {
    ...
    for (int i = 0; i < NUM_FILTERS; i++) {
      ...
      cv::setIdentity(KF[i].measurementNoiseCov, cv::Scalar(1e-1));
    }
    ...
    for (int i = 0; i < NUM_FILTERS; i++) {
      cout << "KF" << i << ".statePre=" << KF[i].statePre.at<float>(0) << ","
           << KF[i].statePre.at<float>(1) << "\n";
    }
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber sub = nh.subscribe("front/scan", 1, cloud_cb);
