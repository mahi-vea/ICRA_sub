...
class BaseRobot:
    ...
    def __init__(self, X0, robot_spec, dt, ax):
        ...
        if self.robot_spec['model'] == 'SingleIntegrator2D':
            ...
            try:
                from dynamic_unicycle2D_dpcbf import DynamicUnicycle2D_DPCBF
            except ImportError:
                from safe_control.dynamic_env.dynamic_unicycle2D_dpcbf import DynamicUnicycle2D_DPCBF
            self.robot = DynamicUnicycle2D_DPCBF(dt, robot_spec)
            self.yaw = self.X[2, 0]
    ...
    def nominal_input(self, goal, d_min=0.05, k_omega = 2.0, k_a = 1.0, k_v = 1.0):
        ...
        elif self.robot_spec['model'] in ['DynamicUnicycle2D', 'DynamicUnicycle2D_DPCBF', 'KinematicBicycle2D', 'KinematicBicycle2D_C3BF', 'KinematicBicycle2D_DPCBF']:
            return self.robot.nominal_input(self.X, goal, d_min, k_omega, k_a, k_v)
        elif self.robot_spec['model'] == 'DoubleIntegrator2D':
    ...
    def step(self, U, U_att=None):
        ...
        elif self.robot_spec['model'] in ['Unicycle2D', 'DynamicUnicycle2D', 'DynamicUnicycle2D_DPCBF',
                                   'KinematicBicycle2D', 'KinematicBicycle2D_C3BF', 
                                   'KinematicBicycle2D_DPCBF', 'Quad2D', 'VTOL2D']:
            self.yaw = self.X[2, 0]
        elif self.robot_spec['model'] == 'Quad3D':
            self.yaw = self.X[5, 0]
        return self.X
...
if __name__ == "__main__":
    ...
    for i in range(num_steps):
        ...
        if robot_spec['model'] in ['SingleIntegrator2D', 'Unicycle2D', 'KinematicBicycle2D', 'KinematicBicycle2D_C3BF','DynamicUnicycle2D_DPCBF']:
            alpha = 1.0  # 10.0
