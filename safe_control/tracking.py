...
class LocalTrackingController:
    ...
    def get_nearest_obs(self, detected_obs):
        # 1. Handle NoneType or empty input safely
        if detected_obs is None or len(detected_obs) == 0:
            all_obs = self.obs
        else:
            # 2. Combine existing obstacles with new detections
            if len(self.obs) == 0:
                all_obs = np.array(detected_obs)
            else:
                all_obs = np.vstack((self.obs, detected_obs))

        # 3. Early exit if there are no obstacles at all
        if all_obs is None or len(all_obs) == 0:
            return None

        # 4. Ensure all_obs is 2D (Nx3) even if there is only one obstacle
        if all_obs.ndim == 1:
            all_obs = all_obs.reshape(1, -1)

        # 5. Distance Calculation
        # all_obs[:, :2] are (x, y) coordinates
        # all_obs[:, 2] is the radius
        robot_pos = self.robot.get_position()
        
        # Calculate Euclidean distance to the center of each obstacle
        distances = np.linalg.norm(all_obs[:, :2] - robot_pos, axis=1)
        
        # 6. Find index of the nearest edge (Distance to center - Radius)
        # This prevents large obstacles from being ignored just because their center is far
        min_distance_index = np.argmin(distances - all_obs[:, 2])
        
        nearest_obstacle = all_obs[min_distance_index]
        
        # Return as a column vector (consistent with your original code)
        return nearest_obstacle.reshape(-1, 1)
