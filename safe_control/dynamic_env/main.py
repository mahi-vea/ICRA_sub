...
class LocalTrackingControllerDyn(LocalTrackingController):
    ...
    def control_step(self):
        ...
        '''
        # print(f"[DEBUG] robot type: {type(self.robot)}, robot_spec model: {self.robot_spec['model']}")
        ...
        if self.state_machine == 'rotate':
            ...
            elif self.robot_spec['model'] in ['Unicycle2D', 'DynamicUnicycle2D','DynamicUnicycle2D_DPCBF', 'KinematicBicycle2D', 'KinematicBicycle2D_C3BF', 'KinematicBicycle2D_DPCBF', 'Quad2D', 'VTOL2D']:
        ...
        # if self.state_machine == 'stop':
        #     u_ref = self.robot.stop()
        # elif self.state_machine == 'rotate':
        #     goal_angle = np.arctan2(self.goal[1] - self.robot.X[1, 0],
        #                             self.goal[0] - self.robot.X[0, 0])
        #     if self.robot_spec['model'] in ['SingleIntegrator2D', 'DoubleIntegrator2D']:
        #         self.u_att = self.robot.rotate_to(goal_angle)
        ...
        #     else:
        #         u_ref = self.robot.nominal_input(self.goal)
                # print(f"[DEBUG] nominal_input returned: {u_ref}")

        # 4. Update the CBF constraints & 5. Solve the control problem
        # print(f"[DEBUG] state_machine={self.state_machine}, goal={self.goal}, u_ref={u_ref}")
...
def single_agent_main(controller_type):
    dt = 0.05
    model = 'DynamicUnicycle2D_DPCBF' # SingleIntegrator2D, DoubleIntegrator2D, DynamicUnicycle2D, KinematicBicycle2D, KinematicBicycle2D_C3BF, KinematicBicycle2D_DPCBF, Quad2D
    ...
    waypoints = [
    [1,  7.5, 0],   # start — center height
    [6,  7.5, 0],   # approach the wall straight
    [10, 3.0, 0],   # dip DOWN hard — robot must rotate ~-45°
    [13, 6.5, 0],   # squeeze through the gap — rotate back up ~+45°
    [16, 9.0, 0],   # arc upward past the wall — rotate up
    [20, 7.5, 0],   # settle back to center — final goal
    ]

    # Wall of obstacles at x=12, from y=0 to y=5.5 (bottom block)
    # and y=7.5 to y=15 (top block) — gap is between y=5.5 and y=7.5
    # Moving: top block drifts up, bottom block drifts down → gap widens/closes over time

    # known_obs = np.array([
    #     # --- TOP WALL (drifts upward slowly) ---
    #     [12.0,  8.0, 0.5,  0.0,  0.2],
    #     [12.0, 10.0, 0.5,  0.0,  0.2],
    #     [12.0, 12.0, 0.5,  0.0,  0.2],
    #     [12.0, 14.0, 0.5,  0.0,  0.2],
    #     # --- BOTTOM WALL (drifts downward slowly) ---
    #     [12.0,  5.0, 0.5,  0.0, -0.2],
    #     [12.0,  3.0, 0.5,  0.0, -0.2],
    #     [12.0,  1.0, 0.5,  0.0, -0.2],
    # ])
    ...
    for i, obs_info in enumerate(known_obs):
        ...
        is_superellipsoid = 0   # <-- explicit flag
        dynamic_obs.append([ox, oy, r, vx, vy, y_min, y_max, is_superellipsoid])
    ...
    if known_obs.shape[1] != 8:
        known_obs = np.hstack((known_obs, np.zeros((known_obs.shape[0], 1))))
